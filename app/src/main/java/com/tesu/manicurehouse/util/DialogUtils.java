...
//		final AlertDialog alertDialog = new AlertDialog.Builder(ctx).create();
	 ...
	 *  +点击事件
	 *  */
	public static Dialog showInputAlertDialog(Context ctx, OnClickListener listener,String title) {
		Builder builder = new Builder(ctx);
...
//				.findViewById(R.id.alert_dialog_content);
		...
		// 为知道了按钮添加事件，执行退出应用操作
		Button tv_roger = (Button) dialogView.findViewById(R.id.tv_roger);
		TextView textView = (TextView) dialogView.findViewById(R.id.alert_dialog_title);
...
//	}
	...
	public static Dialog createInputLoadDialog(Context context, boolean isClickable,String text) {
		LayoutInflater inflater = LayoutInflater.from(context);
		View v = inflater.inflate(R.layout.loading_layout, null);// 得到加载view
		Dialog loadingDialog = new Dialog(context,R.style.loading_dialog);// 创建自定义样式dialog
		loadingDialog.setCancelable(isClickable);// 不可以用“返回键”取消
	...
	// 提示对话框
	public static void showUpdateDialog(Context context, String alertContent,final String url,boolean isForceUpdate) {
...
//				params.height=(int) (d.getHeight()*0.28);
			...
			if(isForceUpdate){
				tv_roger.setVisibility(View.GONE);
			}else {
				tv_roger.setVisibility(View.VISIBLE);
			}
