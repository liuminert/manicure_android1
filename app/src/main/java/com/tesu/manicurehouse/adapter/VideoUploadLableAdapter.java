 ...
 * 修改备注：
 */
public class VideoUploadLableAdapter extends BaseAdapter{
    ...
    public VideoUploadLableAdapter(Context context, List<VideoLableBean> lableResponseList){
        mContext=context;
    ...
    public View getView(int pos, View view, ViewGroup parent) {
        ...
        ViewHolder vh;
        if(view==null){
            view= LayoutInflater.from(mContext).inflate(R.layout.video_upload_lable_item, parent,false);
            vh=new ViewHolder();
            vh.tv_lable = (TextView) view.findViewById(R.id.tv_lable);
            vh.rl_lable = (PercentRelativeLayout) view.findViewById(R.id.rl_lable);
            view.setTag(vh);
        }else{
            vh=(ViewHolder)view.getTag();
        }
        VideoLableBean videoLableBean = lableResponseList.get(pos);
        if(pos == lableResponseList.size()-1){
            vh.rl_lable.setVisibility(View.VISIBLE);
            vh.tv_lable.setVisibility(View.GONE);
        }else {
            vh.rl_lable.setVisibility(View.GONE);
            vh.tv_lable.setVisibility(View.VISIBLE);
        }
    ...
    class ViewHolder{
        TextView tv_lable;
        PercentRelativeLayout rl_lable;
    }
}
