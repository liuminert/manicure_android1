 ...
 */
public class CommunicationCommentsAdapter extends BaseExpandableListAdapter {
    private Context context;
    private List<GetPostCommentListResponse.InformationCommentBean> dataList;
    Handler handler;
    private String url;
    ...
    public CommunicationCommentsAdapter(Context context, List<GetPostCommentListResponse.InformationCommentBean> dataList,Dialog loadingDialog) {
        ...
        handler = new Handler(){
            ...
            public void handleMessage(Message msg) {
                CommunicationCommentsAdapter.this.notifyDataSetChanged();
                LogUtils.e("jsonlalisjdai");
                super.handleMessage(msg);
            }
        };
    }
    public void setDataList(List<GetPostCommentListResponse.InformationCommentBean> dataList){
        this.dataList = dataList;
    }
    public void refresh() {
    ...
    public View getGroupView(final int groupPosition, boolean isExpanded,
        ...
        if(!TextUtils.isEmpty(dataList.get(groupPosition).comment_time)){
            groupHolder.tv_time.setText(DateUtils.milliToSimpleDate(Long.parseLong(dataList.get(groupPosition).comment_time) * 1000));
        }
    ...
    public View getChildView(final int groupPosition, final int childPosition,
        ...
        if(!TextUtils.isEmpty(dataList.get(groupPosition).replyDataList.get(childPosition).reply_time)){
            itemHolder.tv_second_time.setText(DateUtils.milliToSimpleDate(Long.parseLong(dataList.get(groupPosition).replyDataList.get(childPosition).reply_time)*1000));
        }
...
//        if(childPosition==0){
//            itemHolder.rl_bg.setBackgroundDrawable(context.getResources().getDrawable(R.mipmap.bg_communication_comment));
//        }
//        else{
//            itemHolder.rl_bg.setBackgroundColor(Color.parseColor("#f2f3f7"));
//        }
        //设置字体
//        SpannableString styledText = new SpannableString(dataList.get(groupPosition).replyDataList.get(childPosition).content);
//        styledText.setSpan(new TextAppearanceSpan(context, R.style.textsize2), 0,goodsClassLists.get(groupPosition).get(childPosition).indexOf("回"), Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
//        styledText.setSpan(new TextAppearanceSpan(context, R.style.textsize0), goodsClassLists.get(groupPosition).get(childPosition).indexOf("回"), goodsClassLists.get(groupPosition).get(childPosition).indexOf("复") + 1, Spanned.SPAN_EXCLUSIVE_EXCLUSIVE);
//        itemHolder.tv_second_user_name.setText(styledText, TextView.BufferType.SPANNABLE);
        return convertView;
    }
