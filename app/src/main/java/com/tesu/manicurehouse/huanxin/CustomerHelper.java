...
public class CustomerHelper {
    protected static final String TAG = CustomerHelper.class.getSimpleName();
    ...
    public synchronized static CustomerHelper getInstance() {
        if (instance == null) {
        ...
        easeUI.setEaseUserInfoProvider(new EaseUI.EaseUserInfoProvider() {
            ...
            public void setNickAndAvatar(Context context, EMMessage message, ImageView userAvatarView, TextView usernickView) {
                ...
                if (message.direct == EMMessage.Direct.SEND) {
                    //发送方不显示nick
                    String alias =SharedPrefrenceUtils.getString(context,"alias");
                    if(!TextUtils.isEmpty(alias)){
                        EaseUserUtils.setUserNick(alias, usernickView);
                    }

                    String avatar = SharedPrefrenceUtils.getString(context, "avatar");
                    if(!TextUtils.isEmpty(avatar)){
                        Bitmap bitmap = ImageLoader.getInstance().loadImageSync(avatar);
                        if(bitmap != null){
                            EaseUserUtils.setUserAvatar(bitmap,userAvatarView);
                        }
                    }else {
...
//                            usernickView.setText(message.getFrom());
                        ...
                        if (!TextUtils.isEmpty(strUrl)) {
                            ...
                            //正常的string路径
                            Glide.with(context).load(strUrl).diskCacheStrategy(DiskCacheStrategy.ALL).placeholder(com.easemob.easeui.R.drawable.show_logo).into(userAvatarView);
                        } else {
        ...
        easeUI.getNotifier().setNotificationInfoProvider(new EaseNotifier.EaseNotificationInfoProvider() {
            ...
            public Intent getLaunchIntent(EMMessage message) {
                ...
                if (chatType == EMMessage.ChatType.Chat) { // 单聊信息
                    ...
                    intent.putExtra(EaseConstant.EXTRA_CHAT_TYPE, CustomerConstants.CHATTYPE_SINGLE);
    ...
    protected void setGlobalListeners(){
        ...
        connectionListener = new EMConnectionListener() {
            ...
            public void onConnected() {
                ...
                CustomerHelper.getInstance().notifyForRecevingEvents();
            }
        };
        //注册连接监听
    ...
    protected void onConnectionConflict(){
        ...
        intent.putExtra(CustomerConstants.ACCOUNT_CONFLICT, true);
    ...
    protected void onCurrentAccountRemoved(){
        Intent intent = new Intent(appContext, MainActivity.class);
        intent.addFlags(Intent.FLAG_ACTIVITY_NEW_TASK);
        intent.putExtra(CustomerConstants.ACCOUNT_REMOVED, true);
        appContext.startActivity(intent);
    }
...
//                                Toast.makeText(appContext, intent.getStringExtra("cmd_value"), Toast.LENGTH_SHORT).show();
    ...
    public boolean isRobotMenuMessage(EMMessage message){
        try {
            JSONObject jsonObj = message.getJSONObjectAttribute(CustomerConstants.MESSAGE_ATTR_MSGTYPE);
    ...
    public boolean isEvalMessage(EMMessage message){
        try {
            JSONObject jsonObj = message.getJSONObjectAttribute(CustomerConstants.WEICHAT_MSG);
    ...
    public boolean isPictureTxtMessage(EMMessage message){
        ...
        try {
            jsonObj = message.getJSONObjectAttribute(CustomerConstants.MESSAGE_ATTR_MSGTYPE);
        } catch (EaseMobException e) {
        }
        if(jsonObj == null){
            return false;
    ...
    public JSONObject getAgentInfoByMessage(EMMessage message) {
        try {
            JSONObject jsonWeichat = message.getJSONObjectAttribute(CustomerConstants.WEICHAT_MSG);
            if (jsonWeichat == null) {
                return null;
            }
