...
public class ChatFragment extends EaseChatFragment implements EaseChatFragment.EaseChatFragmentListener {
    ...
    //从详情进来的，发送轨迹跟踪
    private int imgSelectedIndex = CustomerConstants.INTENT_CODE_IMG_SELECTED_DEFAULT;
    protected int messageToIndex = CustomerConstants.MESSAGE_TO_DEFAULT;
    ...
    public void onActivityCreated(Bundle savedInstanceState) {
        ...
        //检查是否是从某个商品详情进来
        imgSelectedIndex = fragmentArgs.getInt(CustomerConstants.INTENT_CODE_IMG_SELECTED_KEY, CustomerConstants.INTENT_CODE_IMG_SELECTED_DEFAULT);
        //判断是默认，还是用技能组（售前、售后）
        messageToIndex = fragmentArgs.getInt(CustomerConstants.MESSAGE_TO_INTENT_EXTRA, CustomerConstants.MESSAGE_TO_DEFAULT);
        currentUserNick = HelpDeskPreferenceUtils.getInstance(getActivity()).getSettingCurrentNick();

        currentMobile = SharedPrefrenceUtils.getString(getActivity(), "mobile_phone");
        currentUserName = SharedPrefrenceUtils.getString(getActivity(),"user_name");
...
//        startActivity(intent);
    ...
    protected void registerExtendMenuItem() {
        ...
        // 增加扩展item
//        inputMenu.registerExtendMenuItem("message", R.drawable.em_icon_answer, ITEM_SHORT_CUT_MESSAGE, extendMenuItemClickListener);
    }
...
//		pointToSkillGroup(message, groupName);
        switch (messageToIndex) {
            case CustomerConstants.MESSAGE_TO_PRE_SALES:
                pointToSkillGroup(message, "shouqian");
                break;
            case CustomerConstants.MESSAGE_TO_AFTER_SALES:
...
//                getActivity().overridePendingTransition(R.anim.activity_open, 0);
    ...
    private void setUserInfoAttribute(EMMessage message) {
        ...
        try {
            ...
            String nickName = SharedPrefrenceUtils.getString(getActivity(),"alias");
            if(!TextUtils.isEmpty(nickName)){
                visitorJson.put("userNicknhaoame", nickName);
            }else {
                visitorJson.put("userNickname", currentUserNick);
            }
            if(!TextUtils.isEmpty(currentUserName)){
                visitorJson.put("trueName", currentUserName);
            }else {
                visitorJson.put("trueName", currentUserNick);
            }
            visitorJson.put("qq", "10000");
            if(!TextUtils.isEmpty(currentMobile)){
                visitorJson.put("phone", currentMobile);
            }else {
    ...
    private void sendPictureTxtMessage(int selectedImgIndex) {
        if (selectedImgIndex == CustomerConstants.INTENT_CODE_IMG_SELECTED_DEFAULT) {
            return;
        }
        ...
        imgSelectedIndex = CustomerConstants.INTENT_CODE_IMG_SELECTED_DEFAULT;
        if (jsonMsgType != null) {
