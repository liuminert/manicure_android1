...
public abstract class EaseChatRow extends LinearLayout {
    ...
    private void setUpBaseView() {
        ...
        if(easeUserInfoProvider != null){
            ...
            if(!TextUtils.isEmpty(message.getFrom()) && message.getFrom().equals("kefu")){
                EaseUserUtils.setUserNick("小美", usernickView);
            }
        }else{
            if(message.direct == Direct.SEND){
                EaseUserUtils.setUserAvatar(context, KefuChatManager.getInstance().getCurrentUser(), userAvatarView);
                //发送方不显示nick
//                EaseUserUtils.setUserNick(KefuChatManager.getInstance().getCurrentUser(), usernickView);
            }else{
